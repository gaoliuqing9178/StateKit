...
const resolvedLayout = computed(() => {
  ...
  if (!requestedLayout || blockMeta.value.supportedLayouts.includes(requestedLayout)) {
...
const mergedProps = computed<BaseStateProps>(() => ({ ...blockMeta.value.defaults, title: props.title ?? blockMeta.value.defaults.title, description: props.description ?? blockMeta.value.defaults.description, tone: props.tone ?? blockMeta.value.defaults.tone ?? "neutral", density: props.density ?? blockMeta.value.defaults.density ?? "cozy", layout: resolvedLayout.value, primaryAction: props.primaryAction ?? blockMeta.value.defaults.primaryAction, secondaryAction: props.secondaryAction ?? blockMeta.value.defaults.secondaryAction }));
</script>
