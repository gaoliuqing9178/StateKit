...
每个 Vue Block 都围绕同一组 props 展开。Block 组件本身只是一个带默认值的 preset，底层统一交给 `StatePresetBlock` 和 `StateBlockShell` 渲染。
...
| `primaryAction` | `StateAction` | 主要 CTA。 |
| `secondaryAction` | `StateAction` | 次要 CTA。 |
...
- `loading: true` 时按钮文案会显示为 `Working...`。
