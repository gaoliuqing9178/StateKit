...
当前代码库已经定义并实现了 18 个 Block，按 6 个类别平均分布：

| 类别 | 数量 | 代表场景 |
| --- | --- | --- |
| `empty` | 3 | 空集合、空搜索、首次创建项目 |
...
- 用户口中的 “onboarding” 在 V1 中没有单独作为类别建模，而是通过 `first-project` 这种 first-run / first-successful-step 形式落在 `empty` 类别里。
- 当前元数据中标记为 `launch` 的优先 Block 有 6 个：`empty-search`、`first-project`、`page-error`、`no-permission`、`upgrade-plan`、`task-success`。

### 2. 平台范围

- V1 只正式面向 Vue 3。
- API 以预制组件为主，例如 `EmptySearchState`、`NoPermissionState`、`UpgradePlanState`。
- 样式通过 `@statekit/vue/styles.css` 引入。
...
- 一组 Block 列表与详情页。
- 一个安装说明页。
- 三个示例页面，用于展示状态 Block 在真实 SaaS 流程中的组合方式。
- 一个 admin 风格 example 工程。
